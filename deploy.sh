...
wait_for_sha() {
  local target="$1" timeout_sec="$2" label="$3"
  local deadline=$(( $(date +%s) + timeout_sec ))
  while (( $(date +%s) < deadline )); do
    local prod_sha
    prod_sha=$(check_prod_sha)
    if [[ "$prod_sha" == "$target" ]]; then
      echo "✅ $label: prod = $prod_sha"
      return 0
    fi
    echo "   ($label) prod=$prod_sha esperado=$target — aguardando…"
    sleep 10
  done
  echo "⏱  $label: timeout."
  return 1
...
if [[ $FORCE_REDEPLOY -eq 0 ]]; then
  ...
  if wait_for_sha "$LOCAL_SHA" 180 "auto-deploy"; then
    exit 0
  fi
...
if wait_for_sha "$LOCAL_SHA" 300 "railway up"; then
  exit 0
fi
